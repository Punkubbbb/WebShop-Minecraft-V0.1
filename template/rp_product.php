<?php
  if(isset($_GET['page']) && $_GET['page'] == 'reward')
  {
    $header_echo = "แลกของรางวัล";
  }
  else
  {
...
<div class="card mb-3 text-white" style="background-color:#2d2d2d;">
	<div class="card-header text-center p-1" style="border-bottom: 3px solid #ff94fd;">
		<span class="text-white" style="display: block;"><i class="fas fa-rewardping-cart"></i>&nbsp;<?php echo $header_echo; ?></span>
	</div>
	<div class="card-body">
    <?php
        ...
        <div class="row mb-3">
          ...
          while($server_bungeecord = $query_bungeecord->fetch_assoc()) {
            if(isset($_GET['category']) && !empty($_GET['category']) && $_GET['category'] != NULL) {
              ...
              echo '</br>';
              echo '</br>';
              echo '<a href="?page=reward&category='.$_GET['category'].'&server='.$server_bungeecord['id'].'" class="btn btn-warning btn-block">เลือกเซิฟเวอร์</a>';
              echo '</div>';
              echo '</div>';
              echo '</div>';
              echo '</div>';
              ...
              echo '</br>';
              echo '</br>';
              echo '<a href="?page=reward&server='.$server_bungeecord['id'].'" class="btn btn-warning btn-block">เลือกเซิฟเวอร์</a>';
              echo '</div>';
              echo '</div>';
              echo '</div>';
              echo '</div>';
      ...
      ?>
              ...
              ?>
                ...
                while($category = $query_category->fetch_assoc()) {
                  ...
                  <div class="col-4 text-center" data-aos="fade-up">
                    <a href="?page=reward&amp;category=<?php echo $category['id']; ?>&server=<?php echo $_GET['server']; ?>" class="btn btn-danger btn-block"><?php echo $category['name']; ?></a>
                  </div>
                  <br><br>
                  <?php
                }
        ...
        <div class="row">
          <?php
            if(isset($_GET['page']) && $_GET['page'] != 'reward')
            {
              $sql_product = 'SELECT * FROM rp_shop ORDER BY id DESC';
            }
            else
            {
                    ...
                    <div class="item" style="margin-bottom: 20px;">
                      ...
                      <div class="item-info">
                        <div class="item-text">
                          ...
                          <a href="?page=rp&id=<?php echo $product['id']; ?>" class="btn btn-warning w-100 mb-1 border-0">แลกซื้อ</a>
                        </div>
                      </div>
                    </div>
                  </div>
