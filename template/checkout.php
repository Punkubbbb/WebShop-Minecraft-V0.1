...
<div class="card mb-3 text-white" style="background-color:#2d2d2d;">
	<div class="card-header text-center p-1" style="border-bottom: 3px solid #ff94fd;">
		<span class="text-white" style="display: block;"><i class="fas fa-shopping-cart"></i>&nbsp;ซื้อสินค้า</span>
	</div>
	<div class="card-body">
	<?php
				...
				<div class="row">
					<?php
						if($query_buy->num_rows <= 0)
						{
							?>
								<div class="col-md-12">
									<h5 class="col-md-12 text-center">ไม่พบสินค้านี้</h5>
								</div>
							<?php
						}
						else
						{
											...
											else
											{
												...
												if($query_rcon_server->num_rows > 0)
												{
													...
													if($rcon->connect())
													{
														...
														if($query_rem_points)
														{
															$activities_action = "ซื้อไอเท็ม (".$buy['name'].")";
										...
										<?php
											...
											else
											{
												...
												if($query_receive->num_rows != 0)
												{
													...
													$sql_sender = 'SELECT * FROM authme WHERE username = "'.$_SESSION['username'].'"';
													$query_sender = $connect->query($sql_sender);
													...
													{
														$time_date = date("Y-m-d H:i");
														$activities_action = "ส่งไอเท็ม (".$buy['name'].") ให้กับ ".$receive_realname;
														$sql_insert_log = 'INSERT INTO activities (uid,username,action,date,topup_amount,transaction,server_id) VALUES ("'.$_SESSION['uid'].'","'.$_SESSION['username'].'","'.$activities_action.'","'.$time_date.'","'.$buy['price'].'","'.$buy['id'].'","'.$rcon_server['name_server'].'")';
														$connect->query($sql_insert_log);
														$sql_send_gift = 'INSERT INTO gift (uid_send,uid_receive,username_sender,username_reciver,date,img,command,name,price,server_id) VALUES ("'.$_SESSION['uid'].'","'.$receive_uid.'","'.$sender_realname.'","'.$receive_realname.'","'.$time_date.'","'.$buy['pic'].'","'.$buy['command'].'","'.$buy['name'].'","'.$buy['price'].'","'.$buy['server_id'].'")';
							...
							<div class="col p-4 d-flex flex-column position-static">
					          	...
					          	<form name="buy" method="POST">
						          	...
						          	<button type="submit" name="btn_buy" class="btn btn-success btn-block mt-3">
						          		<i class="fas fa-shopping-basket"></i> ซื้อสินค้า <?php echo $buy['name']; ?>
									</button>
									<br>
					          		<span class="is-divider" data-content="หรือส่งให้เพื่อน" style="margin: 1.5rem 0;"></span>
									<div class="col-md-12 mb-1">
										<div class="alert alert-warning">
											<i class="fas fa-exclamation-triangle"></i> หากส่งของขวัญให้ตัวเองตัองไปกดรับที่หน้าของขวัญ !
										</div>
									</div>
						          	<div class="row">
						          		<div class="col-md-12">
							          		<input name="gift_username" class="form-control mb-1" type="text" placeholder="ชื่อตัวละครที่จะส่งของขวัญให้!"/>
										</div>
							          	<div class="col-md-12">
							          		<button type="submit" name="btn_gift" class="btn btn-warning btn-block">
							          			<i class="fas fa-gift"></i> ส่งของขวัญให้เพื่อน
							          		</button>
							          	</div>
						          	</div>
					          	</form>
