<?php
	if(isset($_GET['id']) && $_GET['id'] != NULL && $_GET['id'] != "")
	{
		...
		if($query_user->num_rows != 0)
		{
			...
			{
				if($_POST['points'] != NULL && $_POST['points'] != "" && $_POST['status'] != NULL && $_POST['status'] != "")
				{
					$sql_edit = 'UPDATE authme SET email = "'.$_POST['email'].'", points = "'.$_POST['points'].'",rp = "'.$_POST['rp'].'", status = "'.$_POST['status'].'" WHERE id = "'.$user_f['id'].'"';
					...
					if($query_edit)
					{
						...
						$linemessage = "แอดมิน ".$_SESSION['username']." ได้ทำการแก้ไข ".$user_f['realname']." พ้อยต์ จาก ".$user_f['points']." เป็น ".$_POST['points']." แต้มสะสมจาก ".$user_f['rp']." เป็น ".$_POST['rp']." และแก้ไขยศจาก ".$user_f['rank']." เป็น ".$_POST['status']." ในเวลา ".date("H:i:s")." ของวันที่ ".date("d-m-Y");
			...
			<table class="table">
				<thead>
					<tr>
						...
						<th class="text-center text-white">พ้อยต์ที่ได้</th>
						<th class="text-center text-white">วันที่</th>
						<th class="text-center text-white">เติมด้วย</th>
					</tr>
				</thead>
				<tbody>
					<?php
						$sql_user_w = 'SELECT * FROM waiting_topup WHERE id';
						$query_user_w = $connect->query($sql_user_w);
						$i = 0;

						$sql_usera = 'SELECT * FROM authme WHERE id = "'.$user_w['uid'].'"';
						$query_user_a = $connect->query($sql_usera);
						...
						if($query_user_w->num_rows != 0)
						{
							while($user_w = $query_user_w->fetch_assoc())
							{
								...
								echo '
									<tr>
										<td class="text-left text-white">'.$user_w['uid'].'</td>
										<td class="text-center text-white">'.$user_w['username'].'</td>
										<td class="text-center text-white">'.$user_w['topup_amount'].'</td>
										<td class="text-center text-white">'.$user_w['topup_got'].'</td>
										<td class="text-center text-white">'.$user_w['date'].'</td>
										<td class="text-center text-white">'.$user_w['action'].'</td>
									</tr>
								';
							}
						}
